...
class DBHelper {
	...
	static openIndexedDB(callback) {
		...
		idbRequest.onupgradeneeded = event => {
			var restaurantStore = event.currentTarget.result.createObjectStore(
				DBHelper.IDB_RESTAURANT_STORE_NAME,
				{ keyPath: 'id' }
			);

			var reviewStore = event.currentTarget.result.createObjectStore(
				DBHelper.IDB_REVIEW_STORE_NAME,
				{ keyPath: 'id' }
			);
		};
	}
		...
		DBHelper.fetchRestaurants(async (error, restaurants) => {
			try {
				for (let restaurant of restaurants) {
					const reviewsForRestaurant = await DBHelper.fetchRewiewsForRestaurantFromNetwork(
						restaurant
					);
	...
	static fetchRewiewsForRestaurant(callback) {
		...
		DBHelper.fetchReviews((error, reviews) => {
			if (error) {
				...
				const reviewsForRestaurant = reviews.filter(
					review => review.restaurant_id === self.restaurant.id
				);
	...
	static async fetchRewiewsForRestaurantFromNetwork(restaurant) {
		let reviewsForRestaurant = await fetch(
			`${DBHelper.REVIEWS_URL}/?restaurant_id=${restaurant.id}`
		);
